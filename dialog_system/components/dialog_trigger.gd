...
func _next_dialog() -> void:
	...
	if _dialog_ended: 
		print_debug("Dialog ended") # TODO Remover debug
		dialog_manager.end_dialog()
		_timer.stop()
		return
	
	if not dialog_manager.dialog_started:
		print_debug("Dialog handler init") # TODO Remover debug
		dialog_handler.init_dialog()
		_dialog_index = 0
		return
	
	print_debug("Node index: ", _dialog_index) # TODO Remover debug
	if not _dialog_index+1 >= len(dialog_manager.dialogs):
		var node_to_show = dialog_manager.dialogs[_dialog_index+1]
		print_debug("Node to show\n", node_to_show) # TODO Remover debug
